				...
				<tbody>
					...
					<tr>
						...
						<td class="button-aksi-table">
							...
							<a href="/produk/{{$produk->id}}/destroy">Hapus Produk</a>
							<a href="/produk/{{$produk->slug}}" target="_blank">Rincian Produk</a>
						</td>
					</tr>
					@endforeach
				</tbody>
	...
	<div class="modal-dialog modal-lg">
		...
		<div class="modal-content">
			<div class="modal-header" id="new-modal-header">
				...
				<h4 class="modal-title" id="title-moda-new">Add Product</h4>
			</div>
			<div class="modal-body">
				<form action="/produk" method="POST" enctype="multipart/form-data">
					<div class="form-group">
						<div class="col-xs-12 col-sm-5" id="solve-grid">
							<label class="lable-mod" for="exampleInputEmail1">Product Name</label>
						...
						<div class="col-xs-12 col-sm-2" id="solve-grid">
							...
							<input type="text" min="1" name="stok" class="form-control" value="{{ old('stok') }}" id="mod-inputan" placeholder="Stok">
							@if ($errors->has('stok'))
							<span class="help-block">
						...
						<div class="col-xs-12 col-sm-3" id="solve-grid">
							<label class="lable-mod" for="exampleInputEmail1">Price</label>
				...
				<form action="/produk/{{ $produk->id }}" method="POST"  enctype="multipart/form-data">
					<div class="form-group">
						...
						<div class="col-xs-12 col-sm-2" id="solve-grid">
							<label class="lable-mod" for="exampleInputEmail1">Satuan</label>
							<select id="select-satuan" name="satuan">
								<option {{strcasecmp($produk->satuan, 'Pasang') == 0  ? 'selected' : ''}}>Pasang</option>
								<option {{strcasecmp($produk->satuan, 'Lusin') == 0  ? 'selected' : ''}}>Lusin</option>
							</select>
							@if ($errors->has('satuan'))
							<span class="help-block">
								<strong>{{ $errors->first('satuan') }}</strong>
